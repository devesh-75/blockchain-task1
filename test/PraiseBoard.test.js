...
describe("The Praise Board & ThankYouWall - 10 Scored Benchmark Test Cases", function () {
  let praiseBoard;
  let thankYouWall;
  let owner;
  let commuter1;
  ...
  beforeEach(async function () {
    ...
    const PraiseBoardFactory = await hre.ethers.getContractFactory("PraiseBoard");
    praiseBoard = await PraiseBoardFactory.deploy();
    await praiseBoard.waitForDeployment();

    const ThankYouWallFactory = await hre.ethers.getContractFactory("ThankYouWall");
    thankYouWall = await ThankYouWallFactory.deploy();
    await thankYouWall.waitForDeployment();
  });

  describe("PraiseBoard Contract Verification", function () {
    ...
    it("Test Case 4: Contract emits TipReceived, TipSent, and PraiseReceived events upon successful tip", async function () {
    ...
    it("Test Case 8: Reading getTips(), getAllTips(), tips(i), and getTipCount() aggregates multiple supporters", async function () {
      ...
      expect(await praiseBoard.totalTips()).to.equal(3);
      expect(await praiseBoard.getTotalTips()).to.equal(3);
      ...
      const indexedTip = await praiseBoard.tips(0);
      expect(indexedTip.donor).to.equal(commuter1.address);
      expect(indexedTip.note).to.equal("Note 1");
    });
  ...
  describe("ThankYouWall Contract Alias Verification", function () {
    it("Verifies ThankYouWall alias implements full tipping & withdraw features", async function () {
      expect(await thankYouWall.owner()).to.equal(owner.address);

      await thankYouWall.connect(commuter1).tip("Thank you!", { value: hre.ethers.parseEther("0.01") });
      expect(await thankYouWall.getTipCount()).to.equal(1);

      await thankYouWall.connect(owner).withdraw();
      expect(await hre.ethers.provider.getBalance(await thankYouWall.getAddress())).to.equal(0);
    });
  });
});
