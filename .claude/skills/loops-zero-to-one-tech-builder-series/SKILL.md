---
name: loops-zero-to-one-tech-builder-series
description: >-
  Build for the Zero to One: Tech Builder Series on Loops House: ideate with the AI
  ...
  this skill whenever the user mentions Zero to One: Tech Builder Series, this contest, its
...
# Zero to One: Tech Builder Series — Loops House skill

Help the builder compete in ONE event: `zero-to-one-tech-builder-series`. This skill carries the event data, ready-to-run `loops` commands, and the workflow below. Commands come pre-filled with the right slugs — replace only the `<angle-bracket>` placeholders. Never invent or substitute ids: the user has at most one project per event (team membership counts), and the platform resolves it from the session, so no project id appears anywhere in this skill.
...
2. **Orient.** Read the event data below (stage, deadlines, problems), then run `loops project get --event zero-to-one-tech-builder-series` to see where the submission stands.
...
```toon
event:
  slug: zero-to-one-tech-builder-series
  name: "Zero to One: Tech Builder Series"
  tagline: "Don't just learn concepts. Ship scalable, production-ready tech."
  stage: build_open
  stageMeaning: Building phase — submissions are OPEN until the end date
  timezone: Asia/Calcutta
  prizeCurrency: USD
  startsAt: "Aug 8, 2026, 6:00 PM (Asia/Calcutta)"
  submissionDeadline: "Aug 16, 2026, 6:00 AM (Asia/Calcutta)"
  registrationDeadline: "Aug 8, 2026, 3:00 PM (Asia/Calcutta)"
  description: null
problems[1]:
  - title: The Praise Board
    slug: thank-you-wall
    brief: "Ifeoma has kept her city's bus timetables online for three years. The transit authority publishes them as scanned PDFs that go stale without warning, so hers is the site nine thousand people actually check before they leave the house in the morning. She pays the hosting out of her own pocket, and every few weeks somebody emails asking how they can chip in. The tip platforms she's tried take a cut, want tax paperwork she doesn't understand, and half of them won't onboard someone with her address at all. She wants the simplest possible thing: a page where a commuter connects a wallet, sends her a few dollars with a short note, and watches their name go up on a live wall of supporters. Nobody s…"
    successLooksLike: "A commuter connects their wallet, sends a small tip with a note, and immediately sees it appear on a live wall of supporters — and Ifeoma can trust every entry on it, with no platform in the middle."
...
```sh
loops credits --event zero-to-one-tech-builder-series
```
...
```sh
loops ideate --event zero-to-one-tech-builder-series -m "<your prompt>"
loops ideate --event zero-to-one-tech-builder-series -m "<follow-up>"               # same conversation
loops ideate --event zero-to-one-tech-builder-series --withProject -m "<prompt>"    # mentor sees the user's project
loops ideate --event zero-to-one-tech-builder-series --new -m "<fresh start>"       # discard the session first
loops ideate --event zero-to-one-tech-builder-series --problems <problemSlug> -m "<prompt>"   # focus on one problem
loops session --event zero-to-one-tech-builder-series            # show the stored conversation (--clear to delete)
```
...
```sh
# The Praise Board
loops knowledge query --event zero-to-one-tech-builder-series --problem thank-you-wall -q "<your question about The Praise Board>"
```
...
```sh
loops project get --event zero-to-one-tech-builder-series       # current state (exists=false if none yet)
loops project create --event zero-to-one-tech-builder-series --name "<name>" --repoUrl <url> --tagline "<one-liner>"
loops project update --event zero-to-one-tech-builder-series --description "<new description>"
```
...
```sh
loops artifact list --event zero-to-one-tech-builder-series
loops artifact save --event zero-to-one-tech-builder-series --name "<title>" --kind idea --body "<markdown body>"
loops artifact update --event zero-to-one-tech-builder-series --id <artifactId> --body "<updated markdown>"
loops artifact remove --event zero-to-one-tech-builder-series --id <artifactId>
```
...
```sh
# The Praise Board
loops evaluate --event zero-to-one-tech-builder-series --problem thank-you-wall
```
